...
export default function Create({ navigation }: Props) {
  ...
  const [status, setStatus] = useState<Donghua['status']>('Assistindo');
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await addDoc(collection(db, 'users', auth.currentUser.uid, 'donghuas'), newDonghua);
      navigation.goBack();
    } catch (err) {
      Alert.alert("Erro", "Ocorreu um erro ao salvar a obra.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <View style={styles.container}>
      ...
      <View style={styles.form}>
        ...
        <TouchableOpacity style={styles.saveBtn} onPress={handleSave} disabled={loading}>
          {loading ? <ActivityIndicator color="#fff" /> : <Text style={styles.saveBtnText}>Adicionar Obra</Text>}
        </TouchableOpacity>
      </View>
    </View>
  );
